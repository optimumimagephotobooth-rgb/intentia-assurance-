...
function onFormSubmit(e) {
  try {
    // ===== CONFIG =====
    const REGISTER_SHEET_NAME = "Case Register";
    ...
    if (!register) {
      throw new Error("Case Register sheet not found.");
    }
    ...
    const newRow = [
      ...
      useCase,           // C Use Case
      new Date(),        // D Intake Received
      STATUS_INTAKE,     // E Status
      FREEZE_NO,         // F Freeze Confirmed
      "",                // G Final PDF Sent
      "",                // H Internal Notes
      guidance           // I Operator Guidance
    ];
